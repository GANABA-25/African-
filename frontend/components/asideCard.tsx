...
export default function AsideCard({ data, index }: MovieCardProps) {
  return (
    <div className="group relative flex h-25 w-full overflow-hidden rounded-xl border border-white/10 shadow-white/5 shadow-sm cursor-pointer">
      ...
      <div className="relative z-20 flex w-full items-center gap-2">
        ...
        <div className="absolute inset-y-0 left-20 right-0 z-30 flex items-center">
          <div className="w-full space-y-2">
            ...
            <div className="flex items-center gap-2 text-[11px]">
              ...
              {data.type === "Series" && (
                <span className="flex items-center gap-1 rounded-md bg-green-500/15 px-2 py-1 text-green-400">
                  <span>{data.episodes}</span>
                  <span className="uppercase">Ep</span>
                </span>
              )}
            </div>
          </div>
        ...
        <div className="relative ml-auto h-full w-32 shrink-0 overflow-hidden">
          <Image
            src={data.image}
            alt={data.title}
            fill
